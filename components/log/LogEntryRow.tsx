...
export function LogEntryRow({ logEntry }: LogEntryRowProps) {
  ...
  return (
    <motion.article
      layout
      className={`
        relative pl-5 sm:pl-6 border-l-[2px] ${meta.borderClass}
        ${hasLinks ? 'group cursor-default' : ''}
      `}
    ...
    >
      ...
      {/* ── Row 2: title ── */}
      <h3
        className="font-sans font-semibold text-ink leading-[1.25] mb-4"
        style={{ fontSize: 'clamp(0.9375rem, 1.8vw, 1.125rem)' }}
      >
        {logEntry.title}
      </h3>

      {/* ── Row 3: body — first paragraph always visible, rest collapsible ── */}
      <div className="space-y-4">
        <p className="font-sans text-[0.9375rem] text-muted leading-[1.8]">
          {firstPara}
        </p>
          ...
          {expanded && restParas.length > 0 && (
            ...
            >
              {restParas.map((para, i) => (
                <p key={i} className="font-sans text-[0.9375rem] text-muted leading-[1.8]">
                  {para}
                </p>
              ))}
            </motion.div>
        ...
        {hasMore && (
          <button
            ...
            className="font-mono text-[0.48rem] uppercase tracking-[0.18em] text-muted hover:text-ink transition-colors duration-200 flex items-center gap-1.5"
          >
            <motion.span
        ...
        <div className="mt-5 pt-4 border-t border-line flex flex-wrap gap-3">
          {logEntry.linkedLookSlug && (
            <Link
              ...
              className="inline-flex items-center gap-2 font-mono text-[0.5rem] uppercase tracking-[0.18em] text-ink border border-line px-3 py-2 rounded-sm hover:border-ink hover:bg-ink hover:text-paper transition-all duration-200 group/btn"
            >
              <span className="font-mono text-[0.44rem] text-muted group-hover/btn:text-paper/60 transition-colors duration-200">
                look ·
              </span>
              <span>View Look</span>
          ...
          {logEntry.linkedArticleSlug && (
            <Link
              ...
              className="inline-flex items-center gap-2 font-mono text-[0.5rem] uppercase tracking-[0.18em] text-muted border border-line px-3 py-2 rounded-sm hover:border-ink hover:text-ink transition-all duration-200 group/btn"
            >
